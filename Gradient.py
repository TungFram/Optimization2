...
def grad_descent2_const(f, gradf, init_t, alpha):
    ...
    t = init_t.copy()
    print("grad_descent2_const " + str(alpha))
    curve_x =[t]
    max_iter = 1000000
    iter = 0
...
def grad_descent2_split(f, gradf, init_t, alpha):
    ...
    prev_t = init_t-10*EPS
    t = init_t.copy()
    print("grad_descent2_split " + str(alpha))
    curve_x = [t]
    max_iter = 1000000
    iter = 0
    g1 = dfdx(t[0],t[1])
...
alpha = 0.01
init_point=np.array([-4.0, -1.0])
# xs = grad_descent2_split(f, gradf, init_point, alpha)
xs = grad_descent2_const(f, gradf, init_point, alpha)
xs.reshape(-1,1)
fig, ax = plt.subplots(figsize=(6, 6))
X, Y, Z = create_mesh(f)
ax = plot_contour(ax, X, Y, Z, alpha)
